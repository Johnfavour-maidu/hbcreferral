...
const PIE_COLORS = ["#C89A2B", "#F59E0B", "#DC2626", "#3BA55C"];
...
function QuickAction({
  ...
  return (
    ...
    >
      ...
      <span className="text-[13px] font-medium text-brown-dark group-hover:text-gold transition-colors">
        {label}
      </span>
    </a>
  );
...
export default function AdminDashboard() {
  ...
  const pieData = [
    { name: "Verified", value: stats.verifiedReferrals || 1 },
    { name: "Pending", value: stats.pendingReferrals || 1 },
    { name: "Rejected", value: stats.rejectedReferrals || 1 },
  ];
    ...
    <div className="space-y-6 max-w-[1400px] mx-auto">
      ...
      <div>
        ...
        <p className="text-brown-light text-sm mt-1">
          Referral Challenge Management Dashboard
        </p>
        <div className="flex items-center gap-3 mt-3">
          <span className="inline-flex items-center gap-1.5 bg-success/10 text-success text-[11px] font-semibold px-3 py-1.5 rounded-full">
            <span className="w-1.5 h-1.5 bg-success rounded-full animate-pulse" />
            Campaign Active
          </span>
          <span className="text-[12px] text-brown-light/60">
            10 Days Remaining
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 border border-cream-dark">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[11px] text-brown-light/50">Last 7 days</span>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-cream-dark">
          <h3 className="text-[15px] font-bold text-brown-dark mb-4">
            Referral Status
          </h3>
        ...
        <div className="bg-white rounded-2xl p-5 border border-cream-dark">
          ...
          {stats.pendingVerifications && stats.pendingVerifications.length > 0 ? (
            <div className="space-y-2.5">
              {stats.pendingVerifications.slice(0, 4).map((v) => (
                <div
                  key={v.id}
          ...
          <div className="space-y-2">
            {stats.topReferrers && stats.topReferrers.length > 0 ? (
              stats.topReferrers.slice(0, 5).map((r, i) => (
                ...
                >
                  <div className="w-7 h-7 rounded-full bg-gold/10 flex items-center justify-center text-[11px] font-bold text-gold shrink-0">
                    {i + 1}
                  </div>
                  ...
                  <span className="text-[13px] font-bold text-gold shrink-0">
                    {r.verifiedReferrals}
                  </span>
                </div>
              ))
            ) : (
              <p className="text-center py-6 text-brown-light/40 text-[13px]">
                No data yet
              </p>
            )}
          </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-cream-dark">
                <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-brown-light/50">
                  Name
                </th>
                <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-brown-light/50">
                  State
            ...
            <tbody>
              ...
              stats.recentParticipants.length > 0 ? (
                stats.recentParticipants.slice(0, 5).map((p) => (
                  <tr
                    key={p.id}
                    className="border-b border-cream-dark/50 last:border-0 hover:bg-cream/30 transition-colors"
                  >
                    ...
                    <td className="px-5 py-3.5 text-[13px] text-brown-light">
                      {p.state}
                    </td>
                    <td className="px-5 py-3.5 text-[13px] text-brown-light">
                      {p.school}
                    </td>
                    <td className="px-5 py-3.5 text-[13px] font-semibold text-brown-dark">
                      {p.totalReferrals}
                    </td>
                    <td className="px-5 py-3.5">
                ...
                <tr>
                  <td
                    colSpan={6}
                    className="px-5 py-10 text-center text-brown-light/40 text-[13px]"
                  >
                    No participants yet
                  </td>
                </tr>
